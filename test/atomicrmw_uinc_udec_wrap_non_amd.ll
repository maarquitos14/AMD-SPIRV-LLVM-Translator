...
; RUN: sed -e 's/OP/uinc_wrap/' -e 's/TRIPLE/spirv64-unknown-unknown/' %s | llvm-as -o %t.bc
; RUN: not llvm-spirv %t.bc -o %t.spv 2>&1 | FileCheck %s --check-prefix=CHECK-UINC

; RUN: sed -e 's/OP/udec_wrap/' -e 's/TRIPLE/spirv64-unknown-unknown/' %s | llvm-as -o %t.bc
; RUN: not llvm-spirv %t.bc -o %t.spv 2>&1 | FileCheck %s --check-prefix=CHECK-UDEC

; RUN: sed -e 's/OP/uinc_wrap/' -e 's/TRIPLE/spir64-unknown-unknown/' %s | llvm-as -o %t.bc
; RUN: not llvm-spirv %t.bc -o %t.spv 2>&1 | FileCheck %s --check-prefix=CHECK-UINC

; RUN: sed -e 's/OP/udec_wrap/' -e 's/TRIPLE/spir64-unknown-unknown/' %s | llvm-as -o %t.bc
; RUN: not llvm-spirv %t.bc -o %t.spv 2>&1 | FileCheck %s --check-prefix=CHECK-UDEC
